        ...
        (function() {
            ...
            async function hapusProyek(id, nama) {
                ...
                const lanjut = window.InaaiDialog ? await window.InaaiDialog.konfirmasi({
                    judul: 'Hapus Proyek',
                    teks: 'Proyek "' + nama +
                        '" akan dihapus permanen beserta seluruh tugas yang terkait. Tindakan ini tidak bisa dibatalkan.',
                    ok: 'Iya, hapus proyek',
                    jenis: 'bahaya'
...
@section('content')
    <div class="card" data-table id="proyekCard">
        <div class="card-head">
            <div class="card-title"><span class="ct-ico"><svg width="15" height="15" viewBox="0 0 24 24"
                    ...
                    </svg></span>Daftar Proyek</div>
            <div class="card-desc">Seluruh proyek beserta progres dan jumlah kontributornya. Bar progres memuat porsi tiap
                status.</div>
            ...
            <table class="tbl">
                <thead>
                    <tr>
                        ...
                        @include('partials.th-sort', [
                            'label' => 'Tugas',
                            'ikon' => '<path d="M11 12H3M16 6H3M16 18H3"/><path d="m18 9 3 3-3 3"/>',
                        ])
                        @include('partials.th-sort', [
                            'label' => 'Kontributor',
                            'ikon' =>
                        ...
                        @include('partials.th-sort', [
                            'label' => 'Progress',
                            'ikon' => '<path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="M18 17V9M13 17V5M8 17v-3"/>',
                        ])
                        @include('partials.th-sort', [
                            'label' => 'Dibuat',
                        ...
                        <tr data-row data-cari="{{ Str::lower($p['nama']) }}">
                            ...
                            <td data-sort="{{ $p['nama'] }}">
                                <div style="display:flex;align-items:center;gap:8px">
                                    <i
                                        style="width:9px;height:9px;border-radius:3px;background:{{ $p['warna'] }};flex:none"></i>
                                ...
                                <div style="display:flex;align-items:center;gap:9px">
                                    <div class="stack" style="flex:1">
                                        @foreach ($p['segmen'] as $sg)
                                            @if ($sg['w'] > 0)
                                                <i style="width:{{ $sg['w'] }}%;background:{{ $sg['warna'] }}"
                                                    title="{{ $sg['label'] }}: {{ $sg['jumlah'] }}"></i>
                                            @endif
                                        @endforeach
                                    </div>
                                    <span
                                        style="font-size:11.5px;color:var(--muted2);white-space:nowrap">{{ $p['pct'] }}%</span>
                                </div>
                            </td>
                            <td data-sort="{{ $p['dibuat']?->timestamp }}"
                            ...
                            <td>
                                <a href="{{ route('admin.proyek.show', $p['id']) }}" class="btn btn-sm">
                                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                                        ...
                                        stroke-linejoin="round">
                                        <path
                                            d="M2.06 12.35a1 1 0 0 1 0-.7 10.75 10.75 0 0 1 19.87 0 1 1 0 0 1 0 .7 10.75 10.75 0 0 1-19.87 0" />
                                        <circle cx="12" cy="12" r="3" />
                                    </svg>
                                    View
                                </a>
                                <button type="button" class="btn btn-sm" data-hapus-proyek="{{ $p['id'] }}"
                                    data-nama="{{ $p['nama'] }}">
                                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round">
    ...
    @php($projects = \App\Models\Project::orderBy('client_or_rd')->get())
