...
<div class="card">
<div class="card-head">
<div class="card-title"><span class="ct-ico"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.2 15.9A10 10 0 1 1 8.1 2.8"/><path d="M22 12A10 10 0 0 0 12 2v10Z"/></svg></span>Komposisi Status</div>
...
<div class="kb-head">
<span class="kb-nama"><i class="kdot" style="background:{{ $st['warna'] }}"></i>{{ $st['label'] }}</span>
<span class="kb-nilai" style="text-align:right">
<span data-kb-jumlah>{{ $st['jumlah'] }}</span> tugas
<span class="kb-sub" style="display:block" data-kb-proyek>{{ $st['proyek'] }} proyek</span>
</span>
</div>
<div class="kb-track"><i data-kb-bar style="width:{{ $st['persen'] }}%;background:{{ $st['warna'] }}"></i></div>
</div>
@endforeach
...
<div class="card-head tl-head">
<div class="card-title"><span class="ct-ico"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3h18v18H3z"/><path d="M3 9h18M3 15h18M9 3v18"/></svg></span>Daftar Pekerjaan</div>
...
<input type="search" id="dpCari" placeholder="Cari judul, proyek, atau reviewer…" aria-label="Cari pekerjaan">
</label>
...
<div class="dp-sel" id="dpStatusWrap"><select id="dpStatus" data-select data-placeholder="Semua status">
<option value="">Semua status</option>
@foreach(\App\Models\Task::daftarStatus() as $k => $lbl)
<option value="{{ $k }}" data-color="{{ \App\Models\Task::titikStatus($k) }}">{{ $lbl }}</option>
@endforeach
</select></div>
...
<option value="{{ $pr }}" data-color="{{ $warna }}">{{ $pr }}</option>
@endforeach
</select></div>
...
<div class="ktask" draggable="true" data-task="{{ $t->id }}" data-status="{{ $t->status }}" data-proyek="{{ $t->project?->nama }}" data-prioritas="{{ $t->prioritas }}" data-cari="{{ Str::lower($t->judul . ' ' . ($t->project?->nama ?? '') . ' ' . ($t->reviewer?->name ?? '')) }}" data-mulai="{{ $t->mulai?->toDateString() }}" data-selesai="{{ $t->selesai?->toDateString() }}">
<div class="ktask-top">
...
<tr data-row data-status="{{ $t->status }}" data-proyek="{{ $t->project?->nama }}" data-prioritas="{{ $t->prioritas }}" data-cari="{{ Str::lower($t->judul . ' ' . ($t->project?->nama ?? '') . ' ' . ($t->reviewer?->name ?? '')) }}" data-mulai="{{ $t->mulai?->toDateString() }}" data-selesai="{{ $t->selesai?->toDateString() }}">
<td data-no></td>
<td data-sort="{{ $t->judul }}">
<strong class="potong" style="--w:290px" title="{{ $t->judul }}">{{ $t->judul }}</strong>
</td>
<td data-sort="{{ $t->project?->nama }}">
<span class="potong" style="--w:170px" title="{{ $t->project?->nama }}">{{ $t->project?->nama ?? '–' }}</span>
</td>
...
<span class="avatar xs" style="background:{{ $wr['bg'] }};color:{{ $wr['text'] }}">{{ $t->reviewer->inisial() }}</span>
<span class="potong" style="--w:140px" title="{{ $t->reviewer->name }}">{{ $t->reviewer->name }}</span>
</div>
@else
...
<div class="modal">
<form method="POST" action="{{ route('tasks.store') }}" enctype="multipart/form-data" id="taskForm" class="modal-form">@csrf
<input type="hidden" name="_method" id="taskMethod" disabled>
<div class="modal-head">
<div style="flex:1">
<div class="card-title"><span class="ct-ico"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg></span><span id="taskFormJudul">Tugas Baru</span></div>
<div class="card-desc" id="taskFormDesc">Lengkapi detail, lampirkan evidence, lalu assign dan minta review.</div>
</div>
...
<input type="text" name="project_baru" id="taskProjectBaru" placeholder="Isi nama proyek baru" value="{{ old('project_baru') }}">
</div>
</div>
<div class="row2">
<div class="fld">
...
<input type="text" name="mulai" id="taskMulai" required data-datepicker value="{{ old('mulai', now()->toDateString()) }}">
</div>
<div class="fld">
<label>Waktu Selesai</label>
<input type="text" name="selesai" id="taskSelesai" required data-datepicker data-min="{{ old('mulai', now()->toDateString()) }}" value="{{ old('selesai', now()->addWeek()->toDateString()) }}">
</div>
</div>
<div class="row2">
<div class="fld">
...
@if($user->isAdmin())
<select name="user_id" id="taskUser" required data-select data-placeholder="Pilih pegawai">
...
document.querySelectorAll('[data-open-task]').forEach(function(b){b.addEventListener('click',function(){resetFormTambah();m.classList.add('open')})});
...
const dMulai=document.getElementById('taskMulai'),dSelesai=document.getElementById('taskSelesai');
if(dMulai&&dSelesai){
...
dSelesai.dispatchEvent(new Event('change',{bubbles:true}));
}
});
}
@if($errors->any())
...
const drw=document.getElementById('taskDrawer'),drwBody=document.getElementById('drwBody'),
      drwFoot=document.getElementById('drwFoot'),drwJudul=document.getElementById('drwJudul');
const form=document.getElementById('taskForm'),metode=document.getElementById('taskMethod'),
      fJudul=document.getElementById('taskFormJudul'),fDesc=document.getElementById('taskFormDesc'),
...
function tutupDrawer(){drw.classList.remove('open');document.body.style.overflow=''}
drw.addEventListener('click',function(e){if(e.target===drw)tutupDrawer()});
document.querySelectorAll('[data-drw-close]').forEach(b=>b.addEventListener('click',tutupDrawer));
document.addEventListener('keydown',function(e){if(e.key==='Escape'&&drw.classList.contains('open'))tutupDrawer()});

function barisDetail(label,isi){return '<div class="drw-row"><div class="drw-k">'+esc(label)+'</div><div class="drw-v">'+isi+'</div></div>'}
...
let d;
try{d=await ambilTugas(id)}catch(e){drwBody.innerHTML='<div class="empty">Gagal memuat detail tugas.</div>';return}

drwJudul.textContent=d.judul||'Detail Tugas';
const w=d.status_warna||{};
let html='';
html+=barisDetail('Status','<span class="badge" style="background:'+esc(w.bg)+';color:'+esc(w.text)+'">'+esc(d.status_label)+'</span>');
html+=barisDetail('Proyek','<span style="display:inline-flex;align-items:center;gap:7px"><i style="width:8px;height:8px;border-radius:2px;background:'+esc(d.warna||'#f55d14')+';display:block"></i>'+esc(d.proyek||'–')+'</span>');
...
if(d.deskripsi)html+=barisDetail('Deskripsi','<span style="white-space:pre-wrap">'+esc(d.deskripsi)+'</span>');
...
const IKO_PENSIL='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>';
...
drwFoot.innerHTML=
(d.boleh_edit?'<button type="button" class="btn btn-danger" data-drw-hapus="'+d.id+'" data-judul="'+esc(d.judul)+'">'+IKO_SAMPAH+'Hapus</button>':'')+
'<span class="drw-foot-sela"></span>'+
'<button type="button" class="btn" data-drw-close>'+IKO_X+'Tutup</button>'+
(d.boleh_edit?'<button type="button" class="btn btn-primary" data-drw-ubah="'+d.id+'">'+IKO_PENSIL+'Ubah Tugas</button>':'');
...
if(bUbah)bUbah.addEventListener('click',function(){tutupDrawer();bukaUbah(d.id)});
const bHapus=drwFoot.querySelector('[data-drw-hapus]');
if(bHapus)bHapus.addEventListener('click',function(){tutupDrawer();hapusTugas(d.id,d.judul||'')});
}
...
const fld=document.getElementById('taskEvidenceLama'),list=document.getElementById('taskEvidenceList');
...
let d;
try{d=await ambilTugas(id)}catch(e){alert('Gagal memuat data tugas.');return}
if(!d.boleh_edit){alert('Tugas ini hanya bisa diubah oleh admin atau pembuatnya.');return}
...
form.querySelectorAll('[data-datepicker],[data-datetimepicker],[data-timepicker],[data-daterange]').forEach(function(el){
...
document.querySelectorAll('[data-lihat]').forEach(b=>b.addEventListener('click',()=>bukaDrawer(b.dataset.lihat)));
document.querySelectorAll('[data-ubah]').forEach(b=>b.addEventListener('click',()=>bukaUbah(b.dataset.ubah)));

/* ===== Hapus tugas (kartu kanban & baris tabel) ===== */
async function hapusTugas(id,judul){
const lanjut=window.InaaiDialog?await window.InaaiDialog.konfirmasi({
judul:'Hapus Tugas',
teks:'Tugas "'+judul+'" beserta seluruh file evidence-nya akan dihapus permanen. Tindakan ini tidak bisa dibatalkan.',
...
b.addEventListener('click',function(e){e.stopPropagation();hapusTugas(b.dataset.hapus,b.dataset.judul||'')});
});
...
const STATUS_URUT=[...document.querySelectorAll('[data-status-bar]')].map(e=>e.dataset.statusBar);
const WARNA={};
...
const PALET=['#f55d14','#0e7490','#4d7c0f','#db2777','#4338ca','#b45309','#0f766e','#9333ea'];
let rentang='30d', status=null;
...
function jendela(){return window.InaaiTimeline.jendela(rentang)}
...
window.InaaiTimeline.saring(function(t){return !status||t.statusKey===status});
...
row.querySelector('[data-kb-bar]').style.width=(dipakai.length?isi.length/dipakai.length*100:0)+'%';
row.classList.toggle('on',status===k);
});

info.textContent=dipakai.length+' tugas pada rentang ini'+(status?' · disaring per status':'');
gambarDrill(dipakai);
}

function gambarDrill(dipakai){
const pie=document.querySelector('.pie'),lubangN=document.querySelector('.pie-n'),lubangL=document.querySelector('.pie-l');
...
return {warna:WARNA[k],nilai:n};
}),dipakai.length,'tugas');
if(judul)judul.textContent=dipakai.length+' tugas · sebaran seluruh status pekerjaan Anda.';
return;
}
...
const label=(document.querySelector('[data-status-bar="'+status+'"] .kb-nama')||{}).textContent||status;
const grup={};
isi.forEach(function(t){(grup[t.proyek]=grup[t.proyek]||[]).push(t)});
const nama=Object.keys(grup).sort(function(a,b){return grup[b].length-grup[a].length});

// Pie kini menggambarkan sebaran status terpilih per proyek.
gambarPie(nama.map(function(p,i){return {warna:PALET[i%PALET.length],nilai:grup[p].length}}),isi.length,'tugas');
...
const persen=isi.length?Math.round(n/isi.length*1000)/10:0;
const w=maks?n/maks*100:0;
const c=PALET[i%PALET.length];
return '<div class="kb-row" style="cursor:default">'+
'<div class="kb-head"><span class="kb-nama"><i class="kdot" style="background:'+c+'"></i>'+esc(p)+'</span>'+
'<span class="kb-nilai" style="text-align:right">'+n+' tugas'+
'<span class="kb-sub" style="display:block">'+persen+'%</span></span></div>'+
'<div class="kb-track"><i style="width:'+w+'%;background:'+c+'"></i></div></div>';
}).join('');
...
drill.querySelector('[data-drill-tutup]').addEventListener('click',function(){status=null;terapkan()});
}
...
if(!pie)return;
let sudut=0;const potong=[];
bagian.forEach(function(b){
const lebar=total?b.nilai/total*360:0;
if(lebar>0){potong.push(b.warna+' '+sudut.toFixed(3)+'deg '+(sudut+lebar).toFixed(3)+'deg');sudut+=lebar}
});
pie.style.background='conic-gradient('+(potong.length?potong.join(', '):'var(--line3) 0deg 360deg')+')';
const n=pie.querySelector('.pie-n'),l=pie.querySelector('.pie-l');
...
document.querySelectorAll('[data-status-bar]').forEach(function(row){
function pilih(){status=status===row.dataset.statusBar?null:row.dataset.statusBar;terapkan()}
row.addEventListener('click',pilih);
row.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();pilih()}});
});
...
if(el)el.addEventListener('change',function(){setter(el.value);terapkan()});
}
...
cari='';fProyek='';fStatus='';fPrioritas='';
