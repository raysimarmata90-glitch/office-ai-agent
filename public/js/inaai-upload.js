...
(function () {
  ...
  var ICON_FILE = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/></svg>';
  var ICON_IMG = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.1-3.1a2 2 0 0 0-2.8 0L6 21"/></svg>';
  ...
  function InaaiUpload(input) {
    ...
    function render() {
      list.innerHTML = files.map(function (f, i) {
        var err = valid(f);
        var gambar = /^image\//.test(f.type);
        return '<div class="i-up-item' + (err ? ' bad' : '') + '">' +
          '<span class="i-up-fi">' + (gambar ? ICON_IMG : ICON_FILE) + '</span>' +
          '<span class="i-up-meta"><span class="i-up-fn">' + esc(f.name) + '</span>' +
          '<span class="i-up-fs">' + (err ? esc(err) : ukuran(f.size)) + '</span></span>' +
  ...
  window.InaaiUpload = { init: InaaiUpload };
