...
@push('style')
    <style>
        ...
        .deliverable-total.is-valid {
            color: var(--st-done)
        }

        .deliverable-total.is-invalid {
...
@section('content')
    <div class="grid-kpi" style="margin-bottom:14px">
        <div class="kpi">
            <div class="kpi-l">Total Tugas</div>
            <div class="kpi-v">{{ $ringkas['total'] }}</div>
            <div class="kpi-s">{{ $ringkas['pct'] }}% selesai</div>
        </div>
        <div class="kpi">
            <div class="kpi-l">Selesai</div>
            <div class="kpi-v">{{ $ringkas['done'] }}</div>
            <div class="kpi-s">dari {{ $ringkas['total'] }} tugas</div>
        </div>
        <div class="kpi">
            <div class="kpi-l">Sedang Dikerjakan</div>
            <div class="kpi-v">{{ $ringkas['progress'] }}</div>
            <div class="kpi-s">termasuk review</div>
        </div>
        <div class="kpi">
            <div class="kpi-l">To Do</div>
            <div class="kpi-v">{{ $ringkas['todo'] }}</div>
            <div class="kpi-s">belum dimulai</div>
        </div>
    </div>
    ...
    <div class="det-grid">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="info-row"><span
                        class="info-k">Periode</span><span>{{ $project->mulai?->format('d/m/y') ?? '–' }} –
                        {{ $project->selesai?->format('d/m/y') ?? '–' }}</span></div>
                <div class="info-row"><span class="info-k">Kontributor</span><span>{{ $kontributor->count() }} orang</span>
                </div>
                <div class="info-row"><span
                        class="info-k">Dibuat</span><span>{{ $project->created_at?->format('d/m/y, H:i') }}</span></div>
                @if ($project->deskripsi)
                    <div style="margin-top:12px;font-size:13px;color:var(--muted);line-height:1.6">
                        {{ $project->deskripsi }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-head">
                <div class="card-title">Kontributor</div>
            </div>
            <div class="card-body">
                @forelse($kontributor as $k)
                    <div class="kontrib">
                        ...
                        <span style="font-size:11.5px;color:var(--muted2);white-space:nowrap">{{ $k['total'] }} ·
                            {{ $k['pct'] }}%</span>
                    </div>
                @empty
    ...
    <div class="card" style="margin-bottom:14px">
        <div class="card-head">
            <div class="card-title">Pembagian Key Deliverable</div>
            <div class="card-desc">Tentukan bobot setiap key deliverable. Total pembagian harus 100%.</div>
        </div>
        <div class="card-body">
            @if ($project->deliverables->isEmpty())
                <div class="empty">Belum ada key deliverable untuk proyek ini.</div>
            @else
                <form id="deliverablePercentageForm"
                    data-url="{{ route('admin.proyek.deliverables.percentages', $project) }}">
                    @csrf
                    @foreach ($project->deliverables->sortBy('code') as $deliverable)
                        <label class="deliverable-row">
                            <span>
                                <span
                                    class="deliverable-name">{{ $deliverable->code ? '[' . $deliverable->code . '] ' : '' }}{{ $deliverable->deliverable_name }}</span>
                                <span class="deliverable-meta">{{ $deliverable->category }}</span>
                            </span>
                            <span class="deliverable-input">
                                <input type="number" name="percentages[{{ $deliverable->id }}]"
                                    value="{{ $deliverable->completion_percentage }}" min="0" max="100"
                                    step="1" required>
                                <span>%</span>
                            </span>
                        </label>
                    @endforeach
                    <div class="deliverable-total" id="deliverablePercentageTotal"><span>Total</span><span
                            data-total>0%</span></div>
                    <button type="submit" class="btn btn-primary" style="margin-top:12px">Simpan Pembagian</button>
                </form>
            @endif
        </div>
    </div>
                    ...
                    <div class="kcol" data-status="{{ $col['key'] }}">
                        ...
                        <div class="kcol-body">
                            @forelse($col['items'] as $t)
                                ...
                                <div class="ktask" draggable="true" data-id="{{ $t->id }}"
                                    data-status="{{ $t->status }}">
                                    <div class="ktask-top">
                                        <div class="ktask-j">{{ $t->judul }}</div>
                                        <button type="button" class="ico-btn xs" data-lihat="{{ $t->id }}"
                                        ...
                                        <button type="button" class="ico-btn xs" data-ubah="{{ $t->id }}"
                                        ...
                                        <button type="button" class="ico-btn xs" data-hapus="{{ $t->id }}"
                ...
                <table class="tbl">
                    ...
                    <tbody>
                        @foreach ($project->tasks as $t)
                            ...
                            <tr data-row>
                                <td data-no></td>
                                <td data-sort="{{ $t->judul }}"><strong class="potong" style="--w:290px"
                                        title="{{ $t->judul }}">{{ $t->judul }}</strong></td>
                                <td data-sort="{{ $t->user?->name }}"><span class="potong"
                                        style="--w:150px">{{ $t->user?->name ?? '–' }}</span></td>
                                ...
                                <td data-sort="{{ $t->evidences->count() }}" style="white-space:nowrap">
                                    @if ($t->evidences->count())
                                        {{ $t->evidences->count() }}
                                        file{{ $t->evidences->count() > 1 ? 's' : '' }}@else<span
                                            style="color:var(--muted3)">–</span>
                                    @endif
                                </td>
    ...
    @push('script')
        <script>
            (function() {
                const persentaseForm = document.getElementById('deliverablePercentageForm');
                if (persentaseForm) {
                    const totalEl = document.getElementById('deliverablePercentageTotal');
                    const inputSelector = 'input[name^="percentages["]';

                    function perbaruiTotal() {
                        let total = 0;
                        persentaseForm.querySelectorAll(inputSelector).forEach(input => {
                            total += Number(input.value) || 0
                        });
                        totalEl.querySelector('[data-total]').textContent = total + '%';
                        totalEl.classList.toggle('is-valid', total === 100);
                        totalEl.classList.toggle('is-invalid', total !== 100);
                        return total;
                    }
                    persentaseForm.querySelectorAll(inputSelector).forEach(input => input.addEventListener('input',
                        perbaruiTotal));
                    perbaruiTotal();
                    persentaseForm.addEventListener('submit', async function(e) {
                        e.preventDefault();
                        if (perbaruiTotal() !== 100) {
                            window.InaaiToast && window.InaaiToast.galat('Total persentase harus tepat 100%.');
                            return;
                        }
                        try {
                            const r = await fetch(persentaseForm.dataset.url, {
                                method: 'PATCH',
                                headers: {
                                    'X-CSRF-TOKEN': persentaseForm.querySelector('input[name="_token"]')
                                        .value,
                                    'Accept': 'application/json',
                                    'X-Requested-With': 'XMLHttpRequest'
                                },
                                body: new FormData(persentaseForm)
                            });
                            const d = await r.json();
                            if (!r.ok) throw new Error(d.message || 'Gagal menyimpan pembagian.');
                            window.InaaiToast && window.InaaiToast.sukses(d.message ||
                                'Pembagian berhasil disimpan.');
                        } catch (err) {
                            window.InaaiToast && window.InaaiToast.galat(err.message)
                        }
                    });
                }
                ...
                document.querySelectorAll('[data-lihat]').forEach(b => b.addEventListener('click', () => window
                    .InaaiDrawerTugas.buka(b.dataset.lihat, 'report')));
                document.querySelectorAll('[data-ubah]').forEach(b => b.addEventListener('click', () => window
                    .InaaiFormTugas.ubah(b.dataset.ubah)));
                document.querySelectorAll('[data-hapus]').forEach(b => b.addEventListener('click', () => window
                    .InaaiDrawerTugas.hapus(b.dataset.hapus, b.dataset.judul || '')));
            })();
        </script>
    @endpush
